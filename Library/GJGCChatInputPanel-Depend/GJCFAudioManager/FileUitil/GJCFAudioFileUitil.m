...
+ (NSString *)cacheDirectory
{
    ...
    ;
    if (![[NSFileManager defaultManager] fileExistsAtPath:subTempFileDir]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:subTempFileDir withIntermediateDirectories:YES attributes:nil error:nil];
    }
...
{
    NSString *fileName = [NSString stringWithFormat:@"%@.wav",[self currentTimeStampString]];
...
+ (void)setupAudioFileLocalStorePath:(GJCFAudioModel*)audioFile
{
    ...
    NSString *fileName = [NSString stringWithFormat:@"%@.%@",[self currentTimeStampString],audioFile.extensionName];
...
+ (void)setupAudioFileTempEncodeFilePath:(GJCFAudioModel*)audioFile
{
    ...
    NSString *fileName = [NSString stringWithFormat:@"%@.%@",[self currentTimeStampString],audioFile.tempEncodeFileExtensionName];
...
+ (BOOL)saveAudioTempEncodeFileToLocalCacheDir:(GJCFAudioModel*)audioFile
{
    ...
    BOOL copyResult = [[NSFileManager defaultManager] copyItemAtPath:audioFile.tempEncodeFilePath toPath:audioFile.localStorePath error:nil];
    if (copyResult) {
        BOOL isRemove = audioFile.isDeleteWhileFinishConvertToLocalFormate;
        if (isRemove) {
            return [[NSFileManager defaultManager] removeItemAtPath:audioFile.tempEncodeFilePath error:nil];
        }
        return YES;
