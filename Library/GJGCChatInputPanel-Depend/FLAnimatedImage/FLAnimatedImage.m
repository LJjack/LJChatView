...
- (instancetype)init
{
    ...
    if (!animatedImage) {
        NSLog(@"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `FLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data
{
    ...
    if (!hasData) {
        NSLog(@"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            NSLog(@"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            NSLog(@"Supplied data is of type %@ and doesn't seem to be GIF data %@", imageSourceContainerType, data);
            return nil;
        }
        ...
        for (size_t i = 0; i < imageCount; i++) {
            ...
            if (frameImageRef) {
                ...
                if (frameImage) {
                    ...
                    if (!delayTime) {
                        if (i == 0) {
                            NSLog(@"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                            delayTime = @(kDelayTimeIntervalDefault);
                        } else {
                            NSLog(@"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                    ...
                    if ([delayTime floatValue] < ((float)kDelayTimeIntervalMinimum - FLT_EPSILON)) {
                        NSLog(@"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kDelayTimeIntervalMinimum);
                        delayTime = @(kDelayTimeIntervalDefault);
                    }
                    delayTimesMutable[i] = delayTime;
                } else {
                    NSLog(@"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
                }
                CFRelease(frameImageRef);
            } else {
                NSLog(@"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, _imageSource);
            }
        }
        ...
        if (self.frameCount == 0) {
            NSLog(@"Failed to create any valid frames for GIF with properties %@", imageProperties);
            return nil;
        } else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            NSLog(@"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        } else {
...
- (UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index
{
    ...
    if (index >= self.frameCount) {
        NSLog(@"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
        return nil;
    }
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    if ([image isKindOfClass:[UIImage class]]) {
        ...
        NSLog(@"`image` isn't of expected types `UIImage` or `FLAnimatedImage`: %@", image);
    }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize
{
    ...
    NSLog(@"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
...
- (void)resetFrameCacheSizeMaxInternal
{
    ...
    NSLog(@"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    NSLog(@"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)FLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
