...
//    self.itemMargin = 40;
    ...
    self.itemMargin = ([UIScreen mainScreen].bounds.size.width - (self.itemWidth * self.columnCount))/(self.columnCount - 1 + 2);
    /* 初始化菜单 */
    [self initMenuItems];
}
...
- (void)initMenuItems
{
    if ([self.delegate respondsToSelector:@selector(menuPanelRequireCurrentConfigData:)] && self.delegate) {
        ...
        [dataSourceItems enumerateObjectsUsingBlock:^(NSDictionary *item, NSUInteger idx, BOOL *stop) {
            ...
            UIImage  *iconImage = [UIImage imageNamed:item[GJGCChatInputExpandMenuPanelDataSourceIconNormalKey]];
            GJGCChatInputMenuPanelActionType actionType = [item[GJGCChatInputExpandMenuPanelDataSourceActionTypeKey]intValue];
            
            __weak typeof(self) weakSelf = self;
            GJGCChatInputExpandMenuPanelItem *panelItem = [GJGCChatInputExpandMenuPanelItem itemWithTitle:itemTitle iconImageNormal:iconImage actionType:actionType tapBlock:^(GJGCChatInputExpandMenuPanelItem *item) {
