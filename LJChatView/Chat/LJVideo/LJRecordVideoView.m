...
-(void)modifyCaptureProperty:(ChangeFocusSucc)capture {
    ...
    if (!isLock) {
        BJLog(@"锁定锁定出错:%@",error.localizedDescription);
    } else {
...
- (void)getAuthorization {
    switch ([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo]) {
        case AVAuthorizationStatusAuthorized: { //已授权
            ...
            BJLog(@"AVAuthorizationStatusAuthorized");
            [self initialSession];
        }
            break;
        case AVAuthorizationStatusNotDetermined: { //未授权
            
            BJLog(@"AVAuthorizationStatusNotDetermined");
            __weak typeof(self) weakSelf = self;
            //请求权限
            [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if(granted) {
                        BJLog(@"initialsession begin");
...
- (void)addPreviewLayer {
    ...
    BJLog(@"设备的方向 %@",@(_previewLayer.connection.videoOrientation));
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error {
    if (!_isCancelRecord) {
        BJLog(@"didFinishRecordingToOutputFileAtURL--> %@", _savePath);
        ...
        BJLog(@"视频压缩 前 大小 %@M", @([NSData dataWithContentsOfURL:outputFileURL].length / 1024. / 1024.));
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^{
            switch ([exportSession status]) {
                case AVAssetExportSessionStatusFailed:
                    BJLog(@"Export failed: %@", [[exportSession error] localizedDescription]);
                    if (fail) {
                        fail();
                    }
                    break;
                case AVAssetExportSessionStatusCancelled:
                    BJLog(@"Export canceled");
                    if (fail) {
                        fail();
                    }
                    break;
                default:
                    if (succ) {
                        ...
                        BJLog(@"视频压缩 后 大小 %@M", @(data.length / 1024. / 1024.));
                        succ(videoPath);
                    }
                    break;
            }
