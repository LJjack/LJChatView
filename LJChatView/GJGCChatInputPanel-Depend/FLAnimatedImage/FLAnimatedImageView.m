...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            GJGCWeakProxy *weakProxy = [GJGCWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    // Early return!
    if (!self.shouldAnimate) {
        NSLog(@"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (image) {
        NSLog(@"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
        ...
        NSLog(@"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
